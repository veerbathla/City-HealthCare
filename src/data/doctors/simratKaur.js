import simratKaurimage from "../../assets/doctors/simratKaurimage.jpg";

const simratKaur = {
  id: 5,
  slug: "dr-simrat-kaur",

  // ==========================
  // Basic Information
  // ==========================
  name: "Dr. Simrat Kaur",
  image:simratKaurimage,
  designation:
    "Consultant Oral & Maxillofacial Surgeon & Cosmetic Facial Specialist",
  department: "Dentistry",
  speciality: "Oral & Maxillofacial Surgery",
  qualifications: [
    "BDS",
    "MDS (Oral & Maxillofacial Surgery)",
    "FAOMSI",
  ],
  experience: "Highly Experienced",

  // ==========================
  // About
  // ==========================
  about: `Dr. Simrat Kaur is a highly qualified Oral and Maxillofacial Surgeon at City HealthCare Neuro Research and Infertility Centre, Sirsa. She specializes in diagnosing and treating diseases, injuries, and defects affecting the mouth, teeth, jaws, and face while combining functional restoration with aesthetic excellence.

As a Fellow of the Association of Oral and Maxillofacial Surgeons of India (FAOMSI), she provides advanced surgical care for facial trauma, dental implants, jaw surgeries, and cosmetic smile enhancement. Her patient-focused approach ensures precise, pain-free treatment using modern technology and evidence-based techniques.`,

  // ==========================
  // Education
  // ==========================
  education: [
    {
      degree: "FAOMSI",
      institute:
        "Fellow of the Association of Oral and Maxillofacial Surgeons of India",
    },
    {
      degree: "MDS (Oral & Maxillofacial Surgery)",
      institute:
        "Dasmesh Institute of Research & Dental Sciences, Faridkot, Punjab",
    },
    {
      degree: "BDS",
      institute:
        "Guru Nanak Dev Dental College, Sunam, Punjab",
    },
  ],

  // ==========================
  // Fellowship & Training
  // ==========================
  training: [
    "Facial Trauma Surgery",
    "Dental Implantology",
  ],

  // ==========================
  // Publications
  // ==========================
  publications: [],

  // ==========================
  // Expertise
  // ==========================
  expertise: [
    "Oral & Maxillofacial Surgery",
    "Facial Trauma Management",
    "Dental Implant Surgery",
    "Wisdom Tooth Surgery",
    "Jaw Fracture Surgery",
    "Root Canal Treatment (RCT)",
    "Cosmetic Dentistry",
    "Smile Designing",
    "TMJ Disorder Management",
    "Pediatric Dentistry",
  ],

  // ==========================
  // Conditions Treated
  // ==========================
  conditionsTreated: [
    "Jaw Fractures",
    "Facial Injuries",
    "Impacted Wisdom Teeth",
    "Jaw Cysts",
    "Jaw Tumours",
    "TMJ Disorders",
    "Missing Teeth",
    "Tooth Decay",
    "Crooked Teeth",
    "Gum Disease",
    "Bleeding Gums",
    "Receding Gums",
  ],

  // ==========================
  // Procedures
  // ==========================
  procedures: [
    "Dental Implants",
    "Root Canal Treatment (RCT)",
    "Wisdom Tooth Extraction",
    "Jaw Fracture Fixation",
    "Facial Trauma Reconstruction",
    "Smile Designing",
    "Teeth Whitening",
    "Dental Veneers",
    "Crowns & Bridges",
    "Dentures",
    "Orthodontic Braces",
    "Clear Aligners",
    "Scaling & Root Planing",
    "Flap Surgery",
  ],

  // ==========================
  // Highlights
  // ==========================
  highlights: [
    "FAOMSI Fellow",
    "Facial Trauma Specialist",
    "Expert in Oral & Maxillofacial Surgery",
    "Advanced Dental Implant Specialist",
    "Cosmetic Smile Design Expert",
    "Integrated Trauma Care with Neurosurgery & Orthopaedics",
  ],

  // ==========================
  // Memberships
  // ==========================
  memberships: [
    "Association of Oral and Maxillofacial Surgeons of India (AOMSI)",
  ],

  // ==========================
  // Appointment Information
  // ==========================
  appointment: {
    hospital: "City HealthCare Neuro Research and Infertility Centre",
    address: "Near ICICI Bank, Deep Tower, Dabwali Road, Sirsa",
    timings: "Monday - Saturday | 10:00 AM - 06:00 PM",
    emergency: "24×7 Emergency Care Available",
    phone: "+91 9728375033",
    email: "cityhealthcaresirsa@gmail.com",
  },

  // ==========================
  // SEO
  // ==========================
  seo: {
    title:
      "Dr. Simrat Kaur | Best Oral & Maxillofacial Surgeon in Sirsa",
    description:
      "Consultant Oral & Maxillofacial Surgeon & Cosmetic Facial Specialist at City HealthCare Sirsa. Expert in Dental Implants, Facial Trauma Surgery, Root Canal Treatment, Smile Designing & Oral Surgery.",
    keywords: [
      "Dentist Sirsa",
      "Oral Surgeon Sirsa",
      "Maxillofacial Surgeon",
      "Dental Implant Specialist",
      "Root Canal Treatment",
      "Smile Designing",
      "Wisdom Tooth Removal",
      "Jaw Fracture Treatment",
      "Cosmetic Dentist Sirsa",
    ],
  },
};

export default simratKaur;